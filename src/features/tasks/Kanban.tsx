...
export default function Kanban({ workspaceId }: { workspaceId: string }) {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto p-4">
      {lists.map((l) => (
        <div key={l.id} className="w-72 shrink-0 rounded-lg border border-neutral-200 bg-white p-3 shadow dark:border-neutral-800 dark:bg-neutral-900">
          <input
          ...
          <div className="space-y-2">
            {(tasksByList[l.id] ?? []).map((t) => (
              <div key={t.id} className="rounded-md border border-neutral-200 bg-white px-2 py-1 text-sm shadow dark:border-neutral-700 dark:bg-neutral-800">
                <input
          ...
          <button className="mt-2 w-full rounded bg-neutral-200 px-2 py-1 text-sm" onClick={() => void addTask(l.id)}>
            + Add task
          </button>
        </div>
      ))}
      <button className="h-10 w-72 shrink-0 rounded-lg border border-dashed border-neutral-300 text-sm text-neutral-500" onClick={() => void addList()}>
        + Add list
      </button>
    </div>
  )

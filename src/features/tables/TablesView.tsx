...
export default function TablesView({ workspaceId }: { workspaceId: string }) {
  ...
  const addTable = async () => {
    ...
    await db.tableRows.put({
      ...
      tableId: id,
      sort: 1,
      cells: { [defaultCols[0].id]: 'First row', [defaultCols[1].id]: 'Todo' },
    })
  ...
  if (!activeTable) {
    return (
      <div className="flex h-[40vh] flex-col items-center justify-center gap-3 border-t border-neutral-200 p-6 text-center text-sm text-neutral-500 dark:border-neutral-800">
        ...
        <button
          className="rounded bg-neutral-200 px-3 py-1 text-neutral-900 dark:bg-neutral-700 dark:text-neutral-100"
    ...
    <div className="flex h-[40vh] flex-col">
      <div className="flex items-center gap-2 border-b border-neutral-200 p-2 dark:border-neutral-800">
        {tables.map(t => (
          <button
            key={t.id}
            className={`rounded px-3 py-1 text-sm ${t.id === activeId ? 'bg-neutral-200 dark:bg-neutral-700' : 'bg-neutral-100 dark:bg-neutral-800'}`}
              ...
              <thead>
                <tr>
                  {columns.map(c => (
                    <th
                      key={c.id}
                      className="border border-neutral-200 bg-neutral-50 px-3 py-1 text-left text-xs font-medium dark:border-neutral-800 dark:bg-neutral-900"
                    >
                      <input
                ...
                {rows.map(r => (
                  <tr key={r.id}>
                    {columns.map(c => (
                      <td
                        key={c.id}
                        className="border border-neutral-200 px-3 py-1 text-sm dark:border-neutral-800"
                      >
                        <input
